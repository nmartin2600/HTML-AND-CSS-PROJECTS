...
function displaySelectedMovieOptions() {
    var movie = document.getElementById('movieSelect')
        .options[document.getElementById('movieSelect').selectedIndex].text;
    var time = document.getElementById('timeSelect')
        .options[document.getElementById('timeSelect').selectedIndex].text;
...
$(document).on("scroll", function () {
    ...
    if ($(document).scrollTop() > 50) {
        ...
        // nav-shrink class to the same HTML element that has
        // nav class
        $("nav").addClass("nav-shrink");
        // Adjust the position of the mobile drop menu
        // to accommodate the new height decrease
        $("div.navbar-collpase").css("margin-top", "-6px");
    }
    else {
        // If the webpage has not been scrolled down or
        // is back at the top the nav-srhink class is removed
        // from the HTML element with the nav class
...
$(document).ready(function () {
    ...
    // and not dropdown-toggle and then also close when an element 
    // with class .dropdown-item (each movie link) has been clicked
